...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, **kwargs):
        ...
        if inputs_embeds is None:
            ...
            mask_prompt_emb = self.word_embeddings(torch.LongTensor([103]).to(device2)).detach()
        ...
        ###
        device2 = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        prompt_zero_embeddings = torch.zeros(int(token_type_embeddings.shape[0]),100,int(token_type_embeddings.shape[2])).to(device2)
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        self.activation_states = hidden_states # .cpu().tolist()

        if self.layer_idx == 6 and self.perturbation_mask is not None:
            print("neuron before noise", hidden_states[0][70][1713]) # print first skill neuron of mrpc

        if self.perturbation_mask is not None:
            for i in range(hidden_states.shape[0]):
                hidden_states[i] += self.perturbation_mask[self.layer_idx]

        if self.layer_idx == 6 and self.perturbation_mask is not None:
            print("noise", self.perturbation_mask[self.layer_idx][70][1713])
            print("neuron after noise", hidden_states[0][70][1713]) # print first skill neuron of mrpc
        return hidden_states
...
class BertLayer(nn.Module):
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        self.activation_states = self.intermediate.activation_states
        return layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            self.activation_states[f"layer {i}"] = layer_module.activation_states
            #self.activation_states_list.append(layer_module.activation_states)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        self.bert.set_perturbation_mask(self.perturbation_mask)
        ...
        self.activation_states = self.bert.activation_states
        #self.activation_states_list = self.bert.activation_states_list
