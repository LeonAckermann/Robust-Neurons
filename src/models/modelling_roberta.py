...
class RobertaEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, **kwargs):
        ...
        if inputs_embeds is None:
            ...
            mask_prompt_emb = self.word_embeddings(torch.LongTensor([50264]).to(device2)).detach()
        ...
        prompt_zero_position_embeddings = torch.zeros(int(position_embeddings.shape[0]),100,int(position_embeddings.shape[2])).to(device2)
...
class RobertaIntermediate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        self.activation_states = hidden_states # .cpu().tolist() # saves activations of intermediate_act_fn with shape (batch, seq_len, hidden_size)
        #if self.layer_idx == 7 and self.perturbation_mask is not None:
        #    print("neuron before noise", self.activation_states[0][80][130])
       
        if self.perturbation_mask is not None:
            for i in range(hidden_states.shape[0]): # iterate over batches
                #hidden_states[i] += self.perturbation_mask[self.layer_idx] # add noise
                hidden_states[i][self.perturbation_mask[self.layer_idx]] = 0
        
        #if self.layer_idx == 7 and self.perturbation_mask is not None:
        #    print("noise", self.perturbation_mask[self.layer_idx][80][130])
            #print("neuron after noise", hidden_states[0][80][130])
        #print("hidden states with intermediate size", hidden_states.shape, hidden_states)
        return hidden_states
    
    def set_layer_idx(self, value):
        self.layer_idx = value
    
    def set_perturbation_mask(self, mask):
        self.perturbation_mask = mask
...
class RobertaOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        #print("hidden states with output size", hidden_states.shape, hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        self.intermediate.set_layer_idx(self.layer_idx)
        #print("layer idx in RobertaLayer", self.layer_idx)
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
        self.activation_states = self.intermediate.activation_states # save intermediate activation states of ff-network
        outputs = (layer_output,) + outputs
        return outputs

    def feed_forward_chunk(self, attention_output):
        self.intermediate.set_perturbation_mask(self.perturbation_mask)
...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_module.set_perturbation_mask(self.perturbation_mask)
            layer_module.set_layer_idx(i)  
            ...
            self.activation_states[f"layer {i}"] = layer_module.activation_states # save intermediate activation states of each layer in dict
            #self.activation_states_list.append(layer_module.activation_states) # save intermediate activation states of each layer in list
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def set_perturbation_mask(self, value):
        self.perturbation_mask = value
    ...
    def forward(
        ...
        **kwargs
        ...
        self.activation_states = self.encoder.activation_states # save intermediate activation states of each layer in dict
        #self.activation_states_list = self.encoder.activation_states_list # save intermediate activation states of each layer in list
...
class RobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        self.roberta.set_perturbation_mask(self.perturbation_mask)
        ...
        self.activation_states = self.roberta.activation_states # save intermediate activation states of each layer in dict
        #self.activation_states_list = self.roberta.activation_states_list # save intermediate activation states of each layer in list
