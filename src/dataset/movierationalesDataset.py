...
class movierationalesDataset(Dataset):
    def __init__(self, config, mode, encoding="utf8", *args, **params):
        ...
        # get validation set of movie rationales from huggingface
        if config.get("eval", "compute_baseline") == "True":
            data = load_dataset('movie_rationales', split='train')
        else:
            data = load_dataset('movie_rationales', split='validation')
        print(data)
        '''
        self.data = []
        for rel in data:
